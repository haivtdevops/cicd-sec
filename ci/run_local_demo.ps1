...
trivy fs --scanners vuln --format json --output (Join-Path $reports "sca.trivy.json") (Join-Path $root "app")
...
docker build -t demo-app:demo -f (Join-Path $root "app/Dockerfile") (Join-Path $root "app")
trivy image --format json --output (Join-Path $reports "image.trivy.json") demo-app:demo
